package com.saltfishsoft.springclouddemo.auth.security.filter;
...
public class IntegrationAuthenticationFilter extends GenericFilterBean implements ApplicationContextAware {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if(requestMatcher.matches(request)){
            //设置集成登录信息
            Map<String,String[]> authParameters = request.getParameterMap();
            String authType = request.getParameter(AUTH_TYPE_PARM_NAME);
            IntegrationAuthentication integrationAuthentication = new IntegrationAuthentication(authType,authParameters);
